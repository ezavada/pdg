...
# 			 revised to work with xcodebuild by Ed Zavada <ed.zavada@gmail.com>
# Version  : 1.3
...
COMPILER_NAME = ["llvm-gcc", "llvm-gcc-4", "cc", "gcc", "g++", "cpp", "c++", "gcc3", "ccache", "ccdist" ]

HIDE_NAME = ["CompileC", "setenv", "cd", "PhaseScriptExecution", "sh", "Build", "ARCHS", "SDKROOT" ]

LINKER_NAME = ["CreateUniversalBinary", "Libtool", "Ld"]

LINK_WARNINGS = ["libtool:", "ld:", "collect2:"]
### Color codes:
#
#  The escape magic is:
...
colordict = { 
			...
			"dcopidl": 		"[0;33m",
			"dcopidl2cpp": 	"[0;33m",
			"normal": 		"[0;0m" # disable bold when finishing
...
def processLine( words ):
	...
	for w in words[1:]:
		if include_is_next: 
			...
			archopt.append(w)
			arch_is_next = 0
		elif srcfile_is_next: 
			srcfile.append(w)
			srcfile_is_next = 0
		elif   w[:2] == "-I" : includes.append( w )
		elif   w[:2] == "-F" : includes.append( w )
		elif w == "-iquote" : 
			includes.append( w )
			include_is_next = 1
		elif w == "-isysroot" : 
...
def usage():
	print "xcodebuild output beautifier. By default, run xcodebuild and beautify its output.\nOptions are:"
...
opt_list = [	
	...
	("-v", "--verbose",     0, "Displays details of compilation, not just summary lines"),
	(""  , "--makeprog" , 	1, "Will run and parse the output of the program <arg> instead of 'xcodebuild'."),
...
#	print opts, args
	...
	for o, a in opts:
		if o == "--help" or o == "-h":
			...
			if sys.stdout.isatty():
				...
				print "Not TTY, Ignoring --use-colors"
				USE_COLOR = 0
		elif o == "--no-colors":
			USE_COLOR = 0
		elif o == "--screenwidth":
	...
	if DEBUG_MAKE:
		# log options of make
		log_file = open( "pretty-xcodebuild.log", "w")
		log_file.write ( "\npretty-xcodebuild: " +  `sys.argv[:]` + '\n' )
		log_file.write ( "\nmake: <<" + make_command + ">>\n" )
		log_file.close()
	...
	try:
		...
		while len(l) :
			...
			if first_word == "/bin/sh":
				...
				processLine( words )
			elif first_word in LINKER_NAME:
				outputline( l, "link" )
			elif first_word in HIDE_NAME:
				first_word = ""
				...
				outputline( l, "depends" )
			elif first_word in LINK_WARNINGS:
				if l.find("has no symbols") == -1:
					outputline( l, "warnlibtool" )
			elif first_word == "**" or first_word == "":
				if l.find("BUILD FAILED") != -1:
					outputline( l, "fatal" )
					ret_val = 1
				else:
					outputline( l, "notice" )
